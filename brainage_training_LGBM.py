...
for m in model:
    ...
    file=savepath+'Output_LGBM_%s.txt'%m
    with open(file, 'w') as text_file:
        ...
        #search = lgbm.XGBRegressor(seed=42)
        search = lgbm.LGBMRegressor(learning_rate=0.05, max_depth=2,num_leaves=50)
        ...
        x_copy_save.to_csv(savepath+'Brainage_LGBM_%s.csv' %m, sep=',',index=None)
        ...
        # Save the model
        result.booster_.save_model(savepath+'LGBM_model.txt')
        # Obtain gain feature importance
        feature_importances = result.feature_importances_
        #feature_importances = result.get_booster().get_score(importance_type='gain')
        feature_names = x.columns
        gain_importance_df = pd.DataFrame({'Feature': feature_names, 'Gain': feature_importances})


        # keys = list(feature_importances.keys())
        # values = list(feature_importances.values())

        data = gain_importance_df.sort_values(by='Gain', ascending=False)
        data.to_csv(savepath+'LGBM_Feature_Importances_%s.csv' %m, sep=',')
